...
// $redact = $_POST['redact']?$_POST['redact']:false;
...
if(isset($_POST['redact'])){ 
    $query_redact = "UPDATE Product SET Name='$Name', Description='$Descr', Category_id='$Categ', Price='$Price', Image='$Image' WHERE Id_product='$idP'"; 
    $result_redact = mysqli_query($con, $query_redact);
    if($result_redact) {
        echo "<script>alert('Данные изменены!'); location.href = 'newTovar.php';</script>";  
    } else {
        echo "<script>alert('Ошибка при редактировании: ".mysqli_error($con)."'); location.href = 'newTovar.php';</script>";  
    }
}
// удаление товара
